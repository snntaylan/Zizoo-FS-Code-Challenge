...
class ProvideItem extends Component {
    ...
    constructor(props) {
        super(props);
        this.state.provideItem = this.props.provideItem;
        if (this.state.provideItem.free_extras) {
            this.state.provideItem.free_extras = this.convertStringToJson(this.state.provideItem.free_extras);
        }
    }

    //handle provide items after component mounting
    componentDidMount() {
        this.setState(this.state.provideItem, () => {
            return this.props.provideItem;
        });
    }


    // to convert provided string to readable string

    convertStringToJson = (str) => {
        return str.replace(',', '000').replace(/[^\w\s]/gi, '').replace('000', ', ');
    };

    // to render single provide item
    render() {
        return (
            <MDBContainer className={'offer-items animated fadeInLeftBig slow'}>
                <MDBCard className={'w-75 mb-4'}>
                    <MDBCardBody className={'container'}>
                        <div className={'row'}>
                            <img className={'col-sm-6 float-left'} src={this.state.image_url} alt={'provide-img'} />
